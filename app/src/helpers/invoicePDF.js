...
export async function invoicePDF(invoice) {
  ...
  const nameWidth =
    (doc.getStringUnitWidth(invoice.bill_from_name) * doc.internal.getFontSize()) /
  ...
  //Business Address
  const addressWidth =
    (doc.getStringUnitWidth(invoice.bill_from_address) * doc.internal.getFontSize()) /
    doc.internal.scaleFactor
  ...
  const phoneNumberWidth =
    (doc.getStringUnitWidth(invoice.bill_from_phone_number) * doc.internal.getFontSize()) /
    doc.internal.scaleFactor
  ...
  if (invoice.notes !== null) {
    ...
    doc.text(String(invoice.notes), 15, notesStartY + 8)
  }
  // Save the PDF
